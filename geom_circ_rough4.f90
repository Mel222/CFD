...
! FIRST STEM BOTTOM VGRID
  ...
  do j = nyv11,nyv21
    do ilist_xz = 1, nlist_s_xz
      ...
      list_ib_s_bot_v(4, ilist) = max(0,j-nyv21+1)                 ! RHS laplacian coefficient 
...
! FIRST STEM BOTTOM UGRID
  ...
  do j = nyu11,nyu21
    do ilist_xz = 1, nlist_s_xz
      ...
      list_ib_s_bot_u(4, ilist) =  max(0,j-nyu21+1)                ! RHS laplacian coefficient 
...
! FIRST STEM TOP VGRID
  ...
  do j = nyv12,nyv22
    ...
    list_ib_s_top_v(4, ilist) = max(0,nyv12-j+1)                    ! RHS laplacian coefficient 
...
! FIRST STEM TOP UGRID
  ...
  do j = nyu12,nyu22
    ...
    list_ib_s_top_u(4, ilist) = max(0,nyu12-j+1)                    ! RHS laplacian coefficient 
...
&                                         + mod(ix+1,2)*dnz*shift_stag-1d0,1d0*Ngal(2,3))  
        ...
        list_ib_s_bot_u(4,ilist+shift) = list_ib_s_bot_u(4,ilist)               ! Lap coef 
...
&                                         + mod(ix+1,2)*dnz*shift_stag-1d0,1d0*Ngal(2,3))  
        ...
        list_ib_s_top_u(4,ilist+shift) = list_ib_s_top_u(4,ilist)               ! Lap coef
      end do
...
&                                         + mod(ix+1,2)*dnz*shift_stag-1d0,1d0*Ngal(2,3))  
        ...
        list_ib_s_bot_v(4,ilist+shift) = list_ib_s_bot_v(4,ilist)               ! Lap coef 
...
&                                         + mod(ix+1,2)*dnz*shift_stag-1d0,1d0*Ngal(2,3))  
        ...
        list_ib_s_top_v(4,ilist+shift) = list_ib_s_top_v(4,ilist)               ! Lap coef
      end do
    end do
  end do
!  end do
